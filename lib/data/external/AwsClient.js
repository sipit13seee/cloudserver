...
const USER_AGENT = 'APN/1.0 Scality/1.0 Zenko/1.0';

class ZenkoService extends AWS.S3 {
    setupRequestListeners(request) {
        super.setupRequestListeners(request);
        request.addListener('build', (req) => {
            req.httpRequest.headers['User-Agent'] = USER_AGENT;
        });
    }
}
...
class AwsClient {
    constructor(config) {
        ...
        this._client = new ZenkoService(this._s3Params);
